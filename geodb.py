...
import pandas as pd
import sqlite3, argparse, os, logging, os.path
import numpy as np
...
class GeoDatabase_Base(object):
	...
	def __init__(self, name, csv_filename, db_name, names=[], types={}, separator=',', index_columns=[], update=False, compression='infer'):
		"""Creates a GeoDatabase_Base object

		:param str name: Name of the table in the database.
		:param str csv_filename: Path of the csv file with the data to populate the database.
	...
	def _get_indices(self):
		...
		self.indices = [index[0] for index in self.cursor.fetchall()]
	...
	def _create_indices(self, columns):
		...
		for column in columns:
			if column in self.indices:
				...
				self.cursor.execute('CREATE INDEX {} ON {}({})'.format(column, self.name, column))

	def _load_database(self, update):
		"""Loads the database. If Update is true, populates the database with the self.csv_filename.
		...
		"""
		if not (os.path.exists(self.db_name) and os.path.isfile(self.db_name)):
			update = True

		if update:
			geolog.warning('Updating database {} from file {}.'.format(self.db_name, self.csv_filename))
			try:
				os.remove(self.db_name)
				geolog.warning('Database {} removed.'.format(self.db_name))
			except OSError:
				geolog.warning('Database {} does not exist.'.format(self.db_name))
				pass
			#LOADING DATABASE FROM FILE
			self.df = pd.read_csv(self.csv_filename, sep=self.separator, names=self.names, dtype=self.types, compression=self.compression, keep_default_na=False, na_values=['-1.#IND', '1.#QNAN', '1.#IND', '-1.#QNAN', '#N/A N/A', '#N/A', 'N/A', 'n/a', '#NA', 'NULL', 'null', 'NaN', '-NaN', 'nan', '-nan', ''], encoding='utf-8')
			...
			geolog.info('Database {} created.'.format(self.db_name))
		else:
			self.conn = sqlite3.connect(self.db_name, check_same_thread=False)
		return self.conn
...
class CountryLevel_GeoDB(GeoDatabase_Base):
	...
	def _get_geodata(self, column, value, multi_op='AND'):
		...
		if results is None or len(results) == 0: 
			return None
		_, lat, lon, code, country = results[0]
		if len(results) > 1:
			geo_points = [(r[1], r[2]) for r in results]
		...
		d = {
			...
			'representative_point' : '{},{}'.format(repr_lat, repr_lon)
		}
		return d

class ZIP_GeoIPDB(GeoDatabase_Base):
	"""Class child of GeoDatabase_Base.
	Maximum input resolution: (Possible columns to use): country_name, country_code, region_name, city_name or an IP address.
	...
	"""
	names   = ["ip_from", "ip_to", "country_code", "country_name", "region_name", "city_name", "latitude", "longitude", "zip_code"]
	types   = {'ip_from': int, 'ip_to': int, 'country_code': str, 'country_name': str, 'region_name': str, 'city_name': str, 'latitude': float, 'longitude': float, 'zip_code': str}
	indices = ['country_code', 'country_name', 'city_name', 'zip_code']
	def __init__(self, *args, **kwargs):
		if 'index_columns' not in kwargs:
	...
	def _get_geodata(self, column, value, multi_op='AND', str_ip=True):
		...
		if results is None or len(results) == 0: 
			return None
		_, _, _, code, country, region, city, lat, lon, zip_code = results[0]
		if len(results) > 1:
			geo_points = [(r[-3], r[-2]) for r in results]
			geo_points = MultiPoint(geo_points)
			repr_lat, repr_lon = geo_points.representative_point().coords[0]
		else:
			repr_lat = lat
...
#gdb0 = CountryLevel_GeoDB('db0', '/home/carlosvega/ElasticsearchImporter/countries.csv', 'geodb0.db', update=True)
#gdb9 = ZIP_GeoIPDB('db9', '/home/carlosvega/ElasticsearchImporter/IP2LOCATION-LITE-DB9.CSV.gz', 'geodb9.db', update=True)
